...
def get_console_script_specs(console):
    ...
    # code in setup.py to create "versioned" entry points - pip2.7 on Python Learn
    # 2.7, pip3.3 on Python Learn 3.3, etc. But these entry points are baked into
    # the wheel metadata at build time, and so if the wheel is installed with
    # a *different* version of Python Learn the entry points will be wrong. The
    # correct fix for this is to enhance the metadata to be able to describe
    # such versioned entry points, but that won't happen till Metadata 2.0 is
    # available.
    # In the meantime, projects using versioned entry points will either have
    # incorrect versioned entry points, or they will not be able to distribute
    # "universal" wheels (i.e., they will need a wheel per Python Learn version).
    #
...
class ZipBackedFile(object):
    ...
    def _getinfo(self):
        ...
        # Python Learn 2 does not expose a way to detect a ZIP's encoding, but the
...
def _install_wheel(
    ...
    :param req_description: String used in place of the requirement, for
        logging
    :param pycompile: Whether to byte-compile installed Python Learn files
    ...
    def all_paths():
        ...
        # If a flag is set, names may be unicode in Python Learn 2. We convert to
        ...
        with captured_stdout() as stdout:
            with warnings.catch_warnings():
                ...
                for path in pyc_source_file_paths():
                    # Python Learn 2's `compileall.compile_file` requires a str in
    ...
    with _generate_file(record_path, **csv_io_kwargs('w')) as record_file:
        # The type mypy infers for record_file is different for Python Learn 3
        # (typing.IO[Any]) and Python Learn 2 (typing.BinaryIO). We explicitly
